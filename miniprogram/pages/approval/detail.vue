<template>
  <view class="detail-container">
    <scroll-view scroll-y class="scroll-content">
      <!-- 状态卡片 -->
      <view class="status-card">
        <view :class="['status-icon', `status-${detail.status}`]">
          <text>{{ getStatusIcon(detail.status) }}</text>
        </view>
        <text class="status-text">{{ statusMap[detail.status] }}</text>
        <text v-if="detail.status === 0 && detail.isTimeout" class="status-tips warning">审批超时，请及时处理</text>
        <text v-if="detail.status === 0" class="status-tips">请尽快完成审批</text>
      </view>

      <!-- 基本信息 -->
      <view class="info-card">
        <view class="card-title">申请信息</view>
        <view class="info-row">
          <text class="info-label">申请单号</text>
          <text class="info-value">{{ detail.applyNo }}</text>
        </view>
        <view class="info-row">
          <text class="info-label">申请人</text>
          <text class="info-value">{{ detail.applicantName }}</text>
        </view>
        <view class="info-row">
          <text class="info-label">联系电话</text>
          <text class="info-value">{{ detail.applicantPhone }}</text>
        </view>
        <view class="info-row">
          <text class="info-label">所属部门</text>
          <text class="info-value">{{ detail.deptName }}</text>
        </view>
        <view class="info-row">
          <text class="info-label">申请时间</text>
          <text class="info-value">{{ detail.applyTime }}</text>
        </view>
        <view class="info-row column">
          <text class="info-label">用途说明</text>
          <text class="info-value">{{ detail.purpose }}</text>
        </view>
      </view>

      <!-- 申请物资 -->
      <view class="info-card">
        <view class="card-title">申请物资</view>
        <view class="material-list">
          <view v-for="(item, index) in detail.details" :key="index" class="material-item">
            <view class="material-header">
              <text class="material-name">{{ item.materialName }}</text>
              <view :class="['stock-badge', item.isStockSufficient ? 'success' : 'danger']">
                <text>{{ item.isStockSufficient ? '库存充足' : '库存不足' }}</text>
              </view>
            </view>
            <view class="material-info">
              <text class="material-spec">规格: {{ item.spec }}</text>
              <text class="material-quantity">申请数量: {{ item.quantity }} {{ item.unit }}</text>
              <text :class="['material-stock', item.isStockSufficient ? '' : 'danger']">
                当前库存: {{ item.currentStock }} {{ item.unit }}
              </text>
              <text v-if="!item.isStockSufficient" class="stock-warning">
                ⚠️ 库存不足，缺少 {{ item.quantity - item.currentStock }} {{ item.unit }}
              </text>
            </view>
          </view>
        </view>
      </view>

      <!-- 库存检查结果 -->
      <view v-if="detail.status === 0" class="check-result-card">
        <view :class="['check-result', allStockSufficient ? 'success' : 'warning']">
          <text class="check-icon">{{ allStockSufficient ? '✓' : '⚠️' }}</text>
          <text class="check-text">
            {{ allStockSufficient ? '所有物资库存充足，可以通过审批' : '部分物资库存不足，请谨慎审批' }}
          </text>
        </view>
      </view>

      <!-- 审批信息 -->
      <view v-if="detail.status !== 0" class="info-card">
        <view class="card-title">审批信息</view>
        <view class="info-row">
          <text class="info-label">审批人</text>
          <text class="info-value">{{ detail.approverName || '-' }}</text>
        </view>
        <view class="info-row">
          <text class="info-label">审批时间</text>
          <text class="info-value">{{ detail.approvalTime || '-' }}</text>
        </view>
        <view v-if="detail.status === 1 && detail.approvalOpinion" class="info-row column">
          <text class="info-label">审批意见</text>
          <text class="info-value success">{{ detail.approvalOpinion }}</text>
        </view>
        <view v-if="detail.status === 2" class="info-row column">
          <text class="info-label">拒绝原因</text>
          <text class="info-value danger">{{ detail.rejectReason }}</text>
        </view>
      </view>

      <view class="safe-area-inset-bottom"></view>
    </scroll-view>

    <!-- 底部操作栏 -->
    <view v-if="showActions" class="action-bar safe-area-inset-bottom">
      <button class="action-btn reject" @click="showRejectModal">拒绝</button>
      <button class="action-btn approve" @click="showApproveModal">通过</button>
    </view>

    <!-- 审批通过弹窗 -->
    <view v-if="approveModalVisible" class="modal-mask" @click="hideApproveModal">
      <view class="modal-content" @click.stop>
        <view class="modal-header">
          <text class="modal-title">审批通过</text>
        </view>
        <view class="modal-body">
          <view class="form-item">
            <text class="form-label">审批意见</text>
            <textarea
              v-model="approvalForm.opinion"
              class="form-textarea"
              placeholder="请输入审批意见（选填）"
              maxlength="200"
            />
          </view>
          <view class="stock-warning-box" v-if="!allStockSufficient">
            <text class="warning-text">⚠️ 部分物资库存不足，通过后将自动创建出库单</text>
          </view>
        </view>
        <view class="modal-footer">
          <button class="modal-btn cancel" @click="hideApproveModal">取消</button>
          <button class="modal-btn confirm" @click="handleApprove">确认通过</button>
        </view>
      </view>
    </view>

    <!-- 审批拒绝弹窗 -->
    <view v-if="rejectModalVisible" class="modal-mask" @click="hideRejectModal">
      <view class="modal-content" @click.stop>
        <view class="modal-header">
          <text class="modal-title">审批拒绝</text>
        </view>
        <view class="modal-body">
          <view class="form-item">
            <text class="form-label">拒绝原因 <text class="required">*</text></text>
            <textarea
              v-model="rejectForm.reason"
              class="form-textarea"
              placeholder="请输入拒绝原因"
              maxlength="200"
            />
          </view>
        </view>
        <view class="modal-footer">
          <button class="modal-btn cancel" @click="hideRejectModal">取消</button>
          <button class="modal-btn confirm danger" @click="handleReject">确认拒绝</button>
        </view>
      </view>
    </view>
  </view>
</template>

<script>
import api from '@/api'
import { mapState } from 'vuex'

export default {
  data() {
    return {
      id: null,
      detail: {
        status: 0,
        details: []
      },
      statusMap: {
        0: '待审批',
        1: '已通过',
        2: '已拒绝',
        3: '已出库',
        4: '已取消'
      },
      approveModalVisible: false,
      rejectModalVisible: false,
      approvalForm: {
        opinion: ''
      },
      rejectForm: {
        reason: ''
      },
      loading: false
    }
  },

  computed: {
    ...mapState(['userInfo']),

    showActions() {
      return this.detail.status === 0 && (this.userInfo.roleCode === 'WAREHOUSE' || this.userInfo.roleCode === 'DEPT_ADMIN')
    },

    allStockSufficient() {
      if (!this.detail.details || this.detail.details.length === 0) return true
      return this.detail.details.every(item => item.isStockSufficient)
    }
  },

  methods: {
    async loadData() {
      if (this.loading) return

      this.loading = true
      uni.showLoading({ title: '加载中...' })

      try {
        const res = await api.apply.getApplyDetail(this.id)

        uni.hideLoading()

        if (res.code === 200) {
          this.detail = res.data

          // 设置标题
          uni.setNavigationBarTitle({
            title: '审批详情'
          })
        }
      } catch (err) {
        uni.hideLoading()
        console.error('加载详情失败', err)
        uni.showToast({
          title: err.message || '加载失败',
          icon: 'none'
        })
      } finally {
        this.loading = false
      }
    },

    getStatusIcon(status) {
      const icons = {
        0: '⏳',
        1: '✓',
        2: '✗',
        3: '✓',
        4: '⊘'
      }
      return icons[status] || '•'
    },

    showApproveModal() {
      this.approveModalVisible = true
    },

    hideApproveModal() {
      this.approveModalVisible = false
      this.approvalForm.opinion = ''
    },

    showRejectModal() {
      this.rejectModalVisible = true
    },

    hideRejectModal() {
      this.rejectModalVisible = false
      this.rejectForm.reason = ''
    },

    async handleApprove() {
      try {
        uni.showLoading({ title: '处理中...' })

        const result = await api.approval.approveApply(this.id, {
          approvalStatus: 1,
          approvalOpinion: this.approvalForm.opinion
        })

        uni.hideLoading()

        if (result.code === 200) {
          uni.showToast({
            title: '审批成功',
            icon: 'success',
            duration: 2000
          })

          this.hideApproveModal()

          setTimeout(() => {
            uni.navigateBack()
          }, 2000)
        }
      } catch (err) {
        uni.hideLoading()
        console.error('审批失败', err)
        uni.showToast({
          title: err.message || '审批失败',
          icon: 'none'
        })
      }
    },

    async handleReject() {
      if (!this.rejectForm.reason.trim()) {
        uni.showToast({
          title: '请输入拒绝原因',
          icon: 'none'
        })
        return
      }

      try {
        uni.showLoading({ title: '处理中...' })

        const result = await api.approval.approveApply(this.id, {
          approvalStatus: 2,
          rejectReason: this.rejectForm.reason
        })

        uni.hideLoading()

        if (result.code === 200) {
          uni.showToast({
            title: '已拒绝',
            icon: 'success',
            duration: 2000
          })

          this.hideRejectModal()

          setTimeout(() => {
            uni.navigateBack()
          }, 2000)
        }
      } catch (err) {
        uni.hideLoading()
        console.error('拒绝失败', err)
        uni.showToast({
          title: err.message || '拒绝失败',
          icon: 'none'
        })
      }
    }
  },

  onLoad(options) {
    if (options.id) {
      this.id = options.id
      this.loadData()
    } else {
      uni.showToast({
        title: '参数错误',
        icon: 'none'
      })
      setTimeout(() => {
        uni.navigateBack()
      }, 1500)
    }
  }
}
</script>

<style lang="scss" scoped>
.detail-container {
  min-height: 100vh;
  background-color: #f5f5f5;
}

.scroll-content {
  height: calc(100vh - 160rpx);
  padding: 24rpx 32rpx;
}

.status-card {
  background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
  border-radius: 16rpx;
  padding: 48rpx 32rpx;
  margin-bottom: 24rpx;
  display: flex;
  flex-direction: column;
  align-items: center;
}

.status-icon {
  width: 120rpx;
  height: 120rpx;
  border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 64rpx;
  margin-bottom: 24rpx;
  background-color: rgba(255, 255, 255, 0.2);

  &.status-0 { color: #faad14; }
  &.status-1 { color: #52c41a; }
  &.status-2 { color: #f5222d; }
}

.status-text {
  font-size: 40rpx;
  font-weight: 600;
  color: #ffffff;
  margin-bottom: 16rpx;
}

.status-tips {
  font-size: 24rpx;
  color: rgba(255, 255, 255, 0.85);

  &.warning {
    color: #faad14;
  }
}

.info-card {
  background-color: #ffffff;
  border-radius: 16rpx;
  padding: 32rpx;
  margin-bottom: 24rpx;
}

.card-title {
  font-size: 32rpx;
  font-weight: 500;
  color: #262626;
  margin-bottom: 24rpx;
  padding-bottom: 16rpx;
  border-bottom: 1rpx solid #f0f0f0;
}

.info-row {
  display: flex;
  justify-content: space-between;
  padding: 16rpx 0;
  border-bottom: 1rpx solid #fafafa;

  &:last-child {
    border-bottom: none;
  }

  &.column {
    flex-direction: column;
  }
}

.info-label {
  font-size: 26rpx;
  color: #8c8c8c;
  width: 160rpx;
  flex-shrink: 0;
}

.info-value {
  flex: 1;
  font-size: 26rpx;
  color: #262626;
  text-align: right;

  &.success {
    color: #52c41a;
    text-align: left;
    margin-top: 8rpx;
  }

  &.danger {
    color: #f5222d;
    text-align: left;
    margin-top: 8rpx;
  }

  .column & {
    text-align: left;
    margin-top: 8rpx;
  }
}

.material-list {
  display: flex;
  flex-direction: column;
  gap: 16rpx;
}

.material-item {
  background-color: #fafafa;
  border-radius: 12rpx;
  padding: 24rpx;
}

.material-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 16rpx;
}

.material-name {
  font-size: 28rpx;
  font-weight: 500;
  color: #262626;
  flex: 1;
}

.stock-badge {
  padding: 4rpx 16rpx;
  border-radius: 8rpx;
  font-size: 20rpx;
  margin-left: 16rpx;

  &.success {
    background-color: #f6ffed;
    color: #52c41a;
  }

  &.danger {
    background-color: #fff1f0;
    color: #f5222d;
  }
}

.material-info {
  display: flex;
  flex-direction: column;
  gap: 8rpx;
}

.material-spec,
.material-quantity,
.material-stock {
  font-size: 24rpx;
  color: #8c8c8c;

  &.danger {
    color: #f5222d;
  }
}

.stock-warning {
  font-size: 22rpx;
  color: #faad14;
  margin-top: 4rpx;
}

.check-result-card {
  margin-bottom: 24rpx;
}

.check-result {
  background-color: #ffffff;
  border-radius: 16rpx;
  padding: 32rpx;
  display: flex;
  align-items: center;
  border-left: 8rpx solid;

  &.success {
    border-color: #52c41a;
  }

  &.warning {
    border-color: #faad14;
  }
}

.check-icon {
  font-size: 48rpx;
  margin-right: 24rpx;
}

.check-text {
  flex: 1;
  font-size: 26rpx;
  color: #595959;
  line-height: 1.6;
}

.action-bar {
  position: fixed;
  bottom: 0;
  left: 0;
  right: 0;
  padding: 24rpx 32rpx;
  background-color: #ffffff;
  box-shadow: 0 -4rpx 16rpx rgba(0, 0, 0, 0.05);
  display: flex;
  gap: 16rpx;
}

.action-btn {
  flex: 1;
  height: 88rpx;
  border: none;
  border-radius: 44rpx;
  font-size: 28rpx;

  &.reject {
    background-color: #fff1f0;
    color: #f5222d;
    border: 1rpx solid #f5222d;
  }

  &.approve {
    background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
    color: #ffffff;
  }

  &::after {
    border: none;
  }
}

.modal-mask {
  position: fixed;
  top: 0;
  left: 0;
  right: 0;
  bottom: 0;
  background-color: rgba(0, 0, 0, 0.6);
  display: flex;
  align-items: center;
  justify-content: center;
  z-index: 1000;
}

.modal-content {
  width: 600rpx;
  background-color: #ffffff;
  border-radius: 16rpx;
  overflow: hidden;
}

.modal-header {
  padding: 32rpx;
  border-bottom: 1rpx solid #f0f0f0;
}

.modal-title {
  font-size: 32rpx;
  font-weight: 500;
  color: #262626;
}

.modal-body {
  padding: 32rpx;
}

.form-item {
  margin-bottom: 24rpx;

  &:last-child {
    margin-bottom: 0;
  }
}

.form-label {
  display: block;
  font-size: 26rpx;
  color: #595959;
  margin-bottom: 16rpx;
}

.required {
  color: #f5222d;
}

.form-textarea {
  width: 100%;
  min-height: 200rpx;
  padding: 16rpx;
  background-color: #fafafa;
  border-radius: 8rpx;
  font-size: 26rpx;
  color: #262626;
  line-height: 1.6;
  box-sizing: border-box;
}

.stock-warning-box {
  padding: 16rpx;
  background-color: #fff7e6;
  border-radius: 8rpx;
  border-left: 4rpx solid #faad14;
}

.warning-text {
  font-size: 24rpx;
  color: #faad14;
  line-height: 1.6;
}

.modal-footer {
  display: flex;
  border-top: 1rpx solid #f0f0f0;
}

.modal-btn {
  flex: 1;
  height: 88rpx;
  border: none;
  background-color: transparent;
  font-size: 28rpx;
  color: #262626;

  &.cancel {
    border-right: 1rpx solid #f0f0f0;
    color: #8c8c8c;
  }

  &.confirm {
    color: #667eea;
    font-weight: 500;

    &.danger {
      color: #f5222d;
    }
  }

  &::after {
    border: none;
  }
}

.safe-area-inset-bottom {
  padding-bottom: env(safe-area-inset-bottom);
}
</style>
